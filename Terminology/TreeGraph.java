...
public class TreeGraph extends JPanel {
	...
	public void paintComponent(Graphics g) {
		...
		// Each term has 2 anchor points, 1 above and 1 below (y) both in the middle of the word (x)
		...
		for (int i = 0; i < layer_strings.length; i++) {
			...
			for (int j = 0; j < words_in_layer.length; j++) {
				...
				// First word is calculated by distance to edge of drawing to start of first word + half the length of first word
				if (word_anchor_x == 0) {
					
					word_anchor_x = anchor_x + g.getFontMetrics().stringWidth(words_in_layer[j])/2;

				// Every other word gets calulated by last anchor point + half the length of last word + distance between words + half the length of current word
				} else {
